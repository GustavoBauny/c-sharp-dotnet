...
namespace CursoCSharp.Excecoes
{
    ...
    class ExcecoesPersonalizadas
    {
        public static int PositivoPar()
        {
            ...
            int valor = random.Next(-10, 30);

            if (valor < 0)
            {
                throw new NegativoException("Exceção: Número negativo.");
            }

            if (valor % 2 == 1)
            {
                throw new ImparException("Exceção: Valor ímpar.");
            }
        ...
        public static void Executar()
        {
            try
            {
                Console.WriteLine($"Número positivo par: {PositivoPar()}");
            }
