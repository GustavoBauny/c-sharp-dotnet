...
namespace CursoCSharp.TopicosAvancados
{
    ...
    class Genericos
    {
        public static void Executar()
        {
            ...
            Console.WriteLine($"Valor: {caixa1.metodoGenerico(33)}");
            Console.WriteLine($"\nTipo: {caixa1.Coisa.GetType()}");

            var caixa2 = new Caixa<string>("Construtor");
            Console.WriteLine($"\nValor: {caixa2.metodoGenerico("Método")}");
            Console.WriteLine($"\nTipo: {caixa2.Coisa.GetType()}");

            CaixaProduto caixa3 = new CaixaProduto();
            Console.WriteLine($"\nNome: {caixa3.Coisa.GetType().Name}");
        }
    }
}
